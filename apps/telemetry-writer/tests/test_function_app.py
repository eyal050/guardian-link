...
def test_logs_and_upserts_valid_event(caplog, _stub_clients):
    ...
    assert kwargs["name"].endswith("p0-42.ndjson")
    # NDJSON body: exactly one line containing the upserted doc.
...
def test_id_is_deterministic_per_partition_offset(_stub_clients):
    ...
    assert blob_container.upload_blob.call_args.kwargs["name"].endswith("p3-100.ndjson")
...
def test_body_metadata_partition_id_overrides_payload(_stub_clients):
...
def test_blob_name_hive_partitioned_by_received_time(monkeypatch, _stub_clients):
    _, blob_container = _stub_clients
    fixed = datetime(2026, 4, 27, 15, 5, 0, tzinfo=timezone.utc)
    ...
    monkeypatch.setattr(function_app, "datetime", _FakeDatetime)
    body = json.dumps({"device_id": "sim-01"}).encode()
    telemetry_writer(_event(body, offset="999", sys_props={"PartitionId": "2"}))
    name = blob_container.upload_blob.call_args.kwargs["name"]
    assert name == "events/year=2026/month=04/day=27/hour=15/p2-999.ndjson"
