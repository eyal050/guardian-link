...
10. **Telemetry-writer slice β: raw events archive to a separate Blob storage account, NDJSON, per-batch.** The writer fans out: per-event Cosmos upsert (slice α+) plus per-batch NDJSON to Blob.
    ...
    - **Per-batch blob, deterministic name.** Trigger flipped to `cardinality="many"`; one block-blob per function invocation, named `events/year=YYYY/month=MM/day=DD/hour=HH/p<partition>-<startOffset>-<endOffset>.ndjson`. Hive-style time partitions so a future Spark/Synapse job can mount this as a partitioned external table without rewriting paths. Offset range in the filename means a redelivered identical batch overwrites the same blob (`overwrite=True`). Per-event blobs would explode the count + cost; append blobs cap at 50k blocks and create cross-instance contention.
    ...
    - **Failure semantics: Cosmos-then-Blob in the same loop, errors propagate.** EH redelivers the whole batch; Cosmos upsert id is `<partition>-<offset>` (slice α+) and the blob name is the offset range, so a redeliver overwrites both deterministically. Caveat: the Functions worker may split a retried batch differently than the original; in that case duplicate blobs land under different offset-range names. Acceptable for dev — the cost ceiling is bounded by the retry rate.
